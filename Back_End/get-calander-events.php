...
switch ($method) {
    case "GET":
        ...
        if($_GET['user_id'] != null){
            ...
            $stmt = $mysqli->prepare("SELECT id, user_id ,group_id, title, location, start_time, end_time, descrip FROM user_calander WHERE user_id = ?");
            $stmt->bind_param("s", $user_id);
        }else if($_GET['group_id'] != null){
            $group_id= $_GET['group_id'];
            $stmt = $mysqli->prepare("SELECT id, user_id ,group_id, title, location, start_time, end_time, descrip FROM user_calander WHERE group_id = ?");
